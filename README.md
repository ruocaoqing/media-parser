...
* **访问控制**：微信登录令牌（`X-WX-Token`）鉴权、管理员配置有效期与积分/会员卡、全局与单平台开关、用户级与平台级两级 QPS 限流、以及调用日志。
...
| :--- | :--- | :--- | :--- |
| **纯API微服务模式** | `API_ONLY=true` | 内部微服务、Bot/下载器后端、本地集成 | **开箱即用，免鉴权**：彻底关闭Web前后端，全接口无需凭证直接调用，零数据库开销，适合高并发与多容器扩展。 |
| **完整运营SaaS模式** | `API_ONLY=false`（默认） | 独立自建站点、微信小程序后端 | **带Web前后台**：提供前台体验页、用户中心、管理后台，支持微信登录令牌鉴权、每日额度+签到余额两层次数扣减与两级QPS限流。 |
...
#### 方式 A：免鉴权调用（微服务模式）
`API_ONLY=true` 时全接口无需任何凭证，直接传入分享链接即可解析：
...
#### 方式 B：微信登录令牌调用（运营模式）
运营模式下接口需带 `X-WX-Token`，令牌由服务端在用户微信登录后签发：

```bash
curl 'http://localhost:8051/api/v1/parse' \
  -H 'X-WX-Token: <登录令牌>' \
  -d 'url=https://v.douyin.com/xxx/'
```

> **提示**：接口同样完整支持 `GET` / `POST`（JSON / Form 表单）两种传参方式，详细协议见 [📖 API接口与协议规范](docs/api.md)。
>
> 🔄 **2026-09-22 修订**：原先还有一条「`?key=<API Key>` / `Authorization: Bearer <API Key>`」的密钥通道，**已整体撤销**。原因见 [docs/api.md §1.1.1](docs/api.md)。
